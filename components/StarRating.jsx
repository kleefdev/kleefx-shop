  ...
  return (
    <div className="flex items-center">
      {Array(fullStars)
        .fill(0)
        .map((_, i) => (
          <Star key={`full-${i}`} filled={true} half={false} />
        ))}
      {halfStar && <Star key="half" filled={false} half={true} />}
      {Array(emptyStars)
        .fill(0)
        .map((_, i) => (
          <Star key={`empty-${i}`} filled={false} half={false} />
        ))}
    </div>
  );
};
